...
class ModelGenerator:
    def __init__(self, enable_nested_partial_orders: bool = True, copy_duplicates: bool = False):
        self.used_as_submodel: list[Union[SilentTransition, Transition]] = []
        self.nested_partial_orders: bool = enable_nested_partial_orders
        self.copy_duplicates: bool = copy_duplicates
        pass

    def activity(self, label: str) -> Transition:
        return Transition(label)

    def silent_transition(self) -> SilentTransition:
        return SilentTransition()

    def create_model(self, node: POWL) -> Union[SilentTransition, Transition]:
        if node is None:
    ...
    def xor(self, *args: POWL) -> OperatorPOWL:
        if len(args) < 2:
    ...
    def loop(self, do : POWL, redo : POWL) -> OperatorPOWL:
    ...
    def partial_order(self, dependencies: list[tuple[POWL, ...] | POWL]) -> StrictPartialOrder:
        list_children: list[POWL] = []
